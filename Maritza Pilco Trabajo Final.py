...
variables = ['n_hijos', 'region', 'sexo', 'condicion_empleo', 'categoria_agua']
# Filtrar los datos para incluir solo niños y niñas de la sierra que tengan valores válidos en la columna 'quintil'
datos.groupby("categoria_agua").size()

datos["region"] = datos["region"].apply(lambda x: "Sierra" if x == 1 else "Costa" if x == 2 else "Oriente")

variables_categoricas = ['region', 'sexo', 'condicion_empleo', 'categoria_agua']
variables_numericas = ['n_hijos']

# Filtrar datos 
poblacion_objetivo = datos[(datos["region"] == "Costa") & (datos["categoria_agua"] )]

# Calcular el tamaño de la muestra en estudio
cantidad_ninos = len(poblacion_objetivo)

# Calcular el conteo de la variable clave (quintil) respecto a esos niños
conteo_categoria_agua = poblacion_objetivo['categoria_agua'].value_counts()

print("Cantidad de niños en la población objetivo (región Costa):", cantidad_ninos)
print("Conteo de la variable 'categoria_agua' respecto a esos niños:")
print(conteo_categoria_agua)
#############################################################
...
# Filtrar los datos para incluir niños y niñas en la Costa que tengan valores válidos en la columna 'quintil'
datos_costa_categoria_agua = datos_limpios[(datos_limpios['region'] == 'Costa') & (datos_limpios['quintil_binario'] == 1)]
...
for i in variables:
    datos_costa_categoria_agua = datos_costa_categoria_agua[~datos_costa_categoria_agua[i].isna()]

# Agrupar los datos por region y categoria agua y contar el número de niños en cada grupo
conteo_por_categoria = datos_costa_categoria_agua.groupby(["region", "categoria_agua"]).size()
print("Conteo de niños por categoría de 'categoria_agua':")
